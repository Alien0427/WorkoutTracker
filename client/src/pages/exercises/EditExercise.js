...
// Options for select inputs (same as CreateExercise)
...
const EditExercise = () => {
  ...
  useEffect(() => {
    if (exercise) {
      // Ensure we only allow editing custom exercises owned by the user
      // This check should ideally happen server-side too, but good practice for UI
      // if (!exercise.isCustom || exercise.user !== userId) { navigate('/exercises'); } 
      ...
      setFormData({
        ...
        equipmentNeeded: exercise.equipmentNeeded || 'none',
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(updateExercise({ id, exerciseData: formData })).then((action) => {
      if (updateExercise.fulfilled.match(action)) {
        dispatch(addNotification({ message: 'Exercise updated successfully!', type: 'success' }));
        navigate(`/exercises/${id}`);
      } else {
        dispatch(addNotification({ message: error || 'Failed to update exercise', type: 'error' }));
      }
    });
  };
      ...
      >
        <Paper elevation={2} sx={{ p: 3, borderRadius: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
            <IconButton onClick={() => navigate(`/exercises/${id}`)} sx={{ mr: 1 }}>
              <ArrowBackIcon />
            </IconButton>
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate>
            <Grid container spacing={3}>
              {/* Form Fields (similar to CreateExercise) */}
            ...
            <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end' }}>
              <Button
                ...
                disabled={loading}
              >
                {loading ? <CircularProgress size={24} /> : 'Update Exercise'}
              </Button>
            </Box>
          </Box>
        </Paper>
